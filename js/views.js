...
export function HomeView(root) {
  ...
  root.append(
    ...
    h("div", { class: "hero home-hero" },
      h("p", { class: "kicker" }, "TeachMeJEE"),
      h("h1", {}, "Study, simulate, repeat."),
      h("p", {}, "Notes, 3D labs, Spline scenes, and a planner.")),
    h("div", { class: "home-actions" },
      lastC ? h("a", { class: "btn btn-primary", href: `#/chapter/${lastC.id}` }, `Resume ${lastC.name}`) : h("a", { class: "btn btn-primary", href: "#/roadmap" }, "Open roadmap"),
      h("a", { class: "btn", href: "#/studio" }, "Spline studio"),
      h("a", { class: "btn", href: "#/simlab" }, "3D labs"),
      h("a", { class: "btn", href: "#/library" }, "Notes")),
...
function renderConstellation(container, completed, starred, state) {
  ...
  function redraw(){
    ...
    for(const n of allNodes){
      ...
      if(showLabels){
        ...
        const lockedLbl = n.status==="locked" && !isSel;
        lbl.setAttribute("font-size", lockedLbl ? Math.max(7, 9*z) : Math.max(8, 11*z));
        lbl.setAttribute("fill","var(--muted)"); lbl.setAttribute("opacity", String((n.status==="completed"?0.85:(lockedLbl?0.32:0.5))*Math.min(fade,localFade)));
  ...
  let lastPtr=null, dragMoved=false;
  svg.addEventListener("pointerdown",(ev)=>{
    const c=ev.target;
    if(c.classList && c.classList.contains("const-node")){ draggingNode=c.getAttribute("data-id"); selectedId=draggingNode; if(localMode) redraw(); lastPtr={x:ev.clientX,y:ev.clientY}; dragMoved=false; try{svg.setPointerCapture(ev.pointerId);}catch{} ev.preventDefault(); return; }
    if(localMode && !(ev.target.closest && ev.target.closest(".const-node"))){ selectedId=null; redraw(); }
  });
  svg.addEventListener("pointermove",(ev)=>{
    if(draggingNode){
      if(lastPtr){
        const ctm=svg.getScreenCTM(); const dx=(ev.clientX-lastPtr.x)/((ctm&&ctm.a)||1), dy=(ev.clientY-lastPtr.y)/((ctm&&ctm.d)||1);
        if(Math.abs(ev.clientX-lastPtr.x)+Math.abs(ev.clientY-lastPtr.y)>2) dragMoved=true;
        positions[draggingNode].x=Math.max(-BOUND_X,Math.min(BOUND_X,positions[draggingNode].x+dx)); positions[draggingNode].y=Math.max(-BOUND_Y,Math.min(BOUND_Y,positions[draggingNode].y+dy)); vz[draggingNode]=0; vvy[draggingNode]=0; paint();
      }
      lastPtr={x:ev.clientX,y:ev.clientY}; return;
    }
    const t=ev.target;
    if(t.classList && t.classList.contains("const-node")){
      const id=t.getAttribute("data-id"); const n=allNodes.find(x=>x.id===id); if(!n) return;
      const r=stage.getBoundingClientRect();
      tooltip.style.display="block"; tooltip.style.left=(ev.clientX-r.left+14)+"px"; tooltip.style.top=(ev.clientY-r.top-10)+"px";
  ...
  const endDrag=()=>{ draggingNode=null; lastPtr=null; };
  svg.addEventListener("pointerup",endDrag);
  svg.addEventListener("pointercancel",endDrag);
  svg.addEventListener("pointerleave",()=>{ draggingNode=null; lastPtr=null; tooltip.style.display="none"; });
  ...
  let saveTimer; svg.addEventListener("pointermove",()=>{ clearTimeout(saveTimer); saveTimer=setTimeout(()=>{ state.constellationPos=positions; save(); },1100); });
  redraw();
}
    ...
    h("div",{class:"stack", style:"gap:14px"},
      ...
      h("div",{class:"labs-grid2"},
        ...
        h("div",{class:"card lab-card"}, h("h3",{},"Haptic Feedback"), h("p",{class:"small muted"},"Vibration on CP correctness."),
          h("div",{class:"lab-demo"}, h("button",{class:"btn btn-sm", onclick:()=>{ Quantum.haptic(true); makeToast("Haptic: correct ✓", true); }}, "Test haptic ✓"), h("button",{class:"btn btn-sm", style:"margin-left:6px", onclick:()=> Quantum.haptic(false)}, "Test haptic ✕")),
          h("span",{class:"tag live"},"LIVE")),
...
export function MoleculeView(root){
  ...
  function memRender(){
    ...
    if(memIdx>=memOrder.length){
      memBox.append(h("div",{class:"quiz-result card"},
        h("div",{class:"big"},`${memScore}/${memOrder.length}`),
        h("p",{class:"muted"},"Shapes and angles drilled. Missed ones return to Flash as reviews."),
        h("div",{style:"margin-top:14px"}, h("button",{class:"btn btn-primary", onclick:()=>{ memShuffle(); memRender(); }}, "Drill again"))));
      return;
    }
    const key=memOrder[memIdx], m=MOL[key];
    const card=h("div",{class:"flash-card", onclick:()=>{ memFlipped=!memFlipped; card.classList.toggle("flipped", memFlipped); }},
      h("div",{class:"flash-inner"},
        h("div",{class:"flash-side flash-front"},
          h("div",{style:"font-size:30px;font-weight:800"}, key),
          h("div",{class:"small faint"},"Shape? Angle? Tap to reveal")),
        h("div",{class:"flash-side flash-back"},
          h("div",{style:"font-weight:700"}, m.name),
          h("div",{class:"small"},"Shape: "+m.shape),
          h("div",{class:"small"},"Angle: "+m.angle))));
    const row=h("div",{class:"row", style:"gap:8px;margin-top:10px;justify-content:center"},
      h("button",{class:"btn btn-primary btn-sm", onclick:()=>{ memScore++; memDone++; srSchedule("mol:"+key, true); memIdx++; memFlipped=false; memRender(); }}, "Got it ✓"),
      h("button",{class:"btn btn-sm", onclick:()=>{ memDone++; srSchedule("mol:"+key, false); memOrder.push(key); memIdx++; memFlipped=false; memRender(); }}, "Again soon"));
    memBox.append(
      h("div",{class:"flash-progress"},
        h("span",{class:"quiz-best"},`Card ${memDone+1} / ${memOrder.length}`),
  ...
  const modeTabs=h("div",{class:"filter-tabs", style:"margin-bottom:4px"},
    ...[["view","Viewer"],["mem","Memorize"]].map(([v,label])=>{
      const b=h("button",{class:`ftab${(v==="mem")===memMode?" on":""}`, onclick:()=>{
        memMode=(v==="mem");
        [...modeTabs.children].forEach(x=>x.classList.toggle("on", x.textContent===label));
        viewWrap.hidden=memMode; memWrap.hidden=!memMode;
        if(memMode){ memShuffle(); memRender(); }
      }}, label);
      return b;
    }));
...
export function NeetView(root) {
  ...
  function modelsTab(rootEl) {
    const MODELS = [
      ...
      { id: "bio-mitosis", name: "Mitosis stages", desc: "Chromosomes condense, align on the plate, then split apart." },
